...
export default function Unrecognized() {
  ...
  const totalActualQty = unrecognized.reduce((sum, v) => sum + (v.actual || 0), 0);
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 pb-20 md:pb-8">
      <div className="max-w-md md:max-w-5xl mx-auto space-y-4 animate-fade-in">
        ...
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-lg md:text-2xl font-bold text-gray-800">Unrecognized Parts</h1>
            <p className="text-xs md:text-sm text-gray-400 mt-0.5">
              Parts that were counted but do not exist in the system.
            </p>
          </div>
          <button onClick={loadUnrecognized}
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          <div className="bg-white rounded-2xl border border-red-50 shadow-sm p-4">
            <div className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Unrecognized Parts</div>
            <div className="text-2xl font-bold text-red-500 mt-1">{formatNumber(unrecognized.length)}</div>
            <div className="text-xs text-gray-400 mt-0.5">not in system</div>
          </div>
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
            <div className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Total Counted Qty</div>
            <div className="text-2xl font-bold text-gray-800 mt-1">{formatNumber(totalActualQty)}</div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
          ...
          {loading && (
            <div className="text-xs text-gray-500 text-center py-6">Loading unrecognized data…</div>
          )}
                          ...
                          return (
                            <React.Fragment key={v.partNo}>
                              ...
                              >
                                ...
                                <td className="px-4 py-3 text-right text-red-400 text-xs">Not found</td>
