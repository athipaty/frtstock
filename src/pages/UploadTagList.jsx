  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 pb-20">
      <div className="max-w-md mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-4 space-y-3 animate-fade-in">
        <div>
          <h2 className="text-base font-semibold text-gray-800">Upload Tag List</h2>
          <p className="text-xs text-gray-500">Upload the list of inventory tags used in the stock count.</p>
        </div>
        ...
        <div className="rounded border bg-gray-50 p-3 text-xs text-gray-700 space-y-1">
          ...
          <div className="font-mono">tagNo</div>
          <div className="text-[11px] text-gray-500">Each row represents one tag number.</div>
        </div>
        ...
        <div className="flex gap-2">
          <button onClick={upload} disabled={!file || uploading}
            className={`flex-1 py-2 rounded text-white text-sm ${!file || uploading ? "bg-gray-400" : "bg-green-600"}`}>
        ...
        {result && (
          <div className="mt-2 rounded border border-green-200 bg-green-50 p-3 text-sm">
            ...
            <div className="mt-1 text-gray-700 text-xs">
              <div><span className="font-medium">Tags uploaded:</span> {result.inserted}</div>
            </div>
          </div>
        )}
