...
export default function Matched() {
  ...
  const DiffCell = ({ value }) => {
    ...
    return (
      <span className={value < 0 ? "text-red-400" : value > 0 ? "text-green-400" : "text-gray-300"}>
      ...
      <div className="max-w-md md:max-w-5xl mx-auto space-y-4 animate-fade-in">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          ...
          <div className={`bg-white rounded-2xl border shadow-sm p-4 transition ${
            showPrevGap ? "border-yellow-400 ring-2 ring-yellow-200" : "border-yellow-100"
          }`}>
            <div className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Had Prev Gap</div>
            <div className="flex items-center gap-2 mt-1">
              <div className="text-2xl font-bold text-yellow-500">{formatNumber(prevGapParts.length)}</div>
              <div className="text-xs text-gray-400 mb-0.5">parts</div>
            </div>
            <button
              onClick={() => { setShowPrevGap((v) => !v); setSearch(""); setOpenPart(null); }}
                            ...
                            <React.Fragment key={v.partNo}>
                              ...
                              >
                                ...
                                <td className="px-4 py-3 text-right text-xs text-gray-400 font-medium">
                                  {diff === 0 ? <span className="text-green-500">0</span> : <DiffCell value={diff} />}
                                </td>
