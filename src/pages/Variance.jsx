...
const isEqual3 = (diff, n1, n2) => {
  ...
  return Math.round(diff) === Math.round(n1) && Math.round(diff) === Math.round(n2);
};
...
export default function Variance() {
  ...
  const consistentShortQty = variances.filter(isShort3).reduce((sum, v) => sum + Math.abs(v.actual - v.system), 0);
  const consistentOverQty = variances.filter(isOver3).reduce((sum, v) => sum + (v.actual - v.system), 0);
    ...
    <div className="min-h-screen bg-gray-50 p-4 md:p-8 pb-20 md:pb-8">
      <div className="max-w-md md:max-w-5xl mx-auto space-y-4 animate-fade-in">
        ...
        <div className="grid grid-cols-3 grid-rows-2 gap-3">
          ...
          {/* Total — spans 2 rows */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 row-span-2 flex flex-col justify-center">
            <div className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Total Parts</div>
            <div className="text-3xl font-bold text-gray-800 mt-2">{formatNumber(variances.length)}</div>
            <div className="text-xs text-gray-400 mt-1">with differences</div>
            <div className="mt-4 pt-4 border-t border-gray-50 space-y-1">
              <div className="flex justify-between text-xs">
                <span className="text-gray-400">Short</span>
                <span className="font-semibold text-red-500">{formatNumber(totalShort)}</span>
              </div>
              <div className="flex justify-between text-xs">
                <span className="text-gray-400">Over</span>
                <span className="font-semibold text-green-500">{formatNumber(totalOver)}</span>
              </div>
            </div>
          </div>
          ...
          <div className={`bg-white rounded-2xl border shadow-sm p-4 transition ${
            consistentFilter === "short3" ? "border-red-400 ring-2 ring-red-200" : "border-red-100"}`}>
            <div className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Short 3× in a Row</div>
            <div className="flex items-end gap-2 mt-1">
              <div className="text-xl font-bold text-red-400">{formatNumber(consistentShort)}</div>
              <div className="text-xs text-gray-400 mb-0.5">parts</div>
            </div>
            <div className="text-xs text-red-400 mt-0.5">qty: {formatNumber(consistentShortQty)}</div>
            <button
              onClick={() => {
                setConsistentFilter(consistentFilter === "short3" ? null : "short3");
                setOpenPart(null);
                setSearch("");
              }}
          ...
          {/* Over 3x */}
          <div className={`bg-white rounded-2xl border shadow-sm p-4 transition ${
            consistentFilter === "over3" ? "border-green-400 ring-2 ring-green-200" : "border-green-100"}`}>
            <div className="text-[11px] font-semibold text-gray-400 uppercase tracking-wide">Over 3× in a Row</div>
            <div className="flex items-end gap-2 mt-1">
              <div className="text-xl font-bold text-green-400">{formatNumber(consistentOver)}</div>
              <div className="text-xs text-gray-400 mb-0.5">parts</div>
            </div>
            <div className="text-xs text-green-400 mt-0.5">qty: +{formatNumber(consistentOverQty)}</div>
            <button
              onClick={() => {
                setConsistentFilter(consistentFilter === "over3" ? null : "over3");
                setOpenPart(null);
                setSearch("");
              }}
              className={`mt-2 w-full text-xs py-1 rounded-lg font-medium transition ${
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
          ...
          {consistentFilter && (
            <div className={`flex items-center justify-between px-3 py-2 rounded-xl text-xs font-medium ${
              ...
              <span>
                {consistentFilter === "short3"
                  ? `🔴 Showing ${filtered.length} parts short 3× in a row`
                  : `🟢 Showing ${filtered.length} parts over 3× in a row`}
              </span>
              <button
                onClick={() => { setConsistentFilter(null); setOpenPart(null); }}
          ...
          {!loading && (
            <>
              ...
              <div className="hidden md:block">
                {filtered.length === 0 ? (
                  <div className="text-xs text-gray-400 text-center py-8 italic">No variance found.</div>
                ) : (
                          ...
                          return (
                            <React.Fragment key={v.partNo}>
                              ...
                              >
                                ...
                                <td className="px-4 py-3 text-right font-medium text-gray-700">{formatNumber(v.actual)}</td>
